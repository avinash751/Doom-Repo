...
public class Astar : MonoBehaviour
{
    ...
    void AddNodesToOpenList()
    {
        for (int i = 0; i < startNodeNeighboursList.Count; i++)
        {
            if (!startNodeNeighboursList[i].isVisited && !openNodeList.Contains(startNodeNeighboursList[i]) && startNodeNeighboursList[i].localPosition != startNode.localPosition)
            {
                ...
                grid.SetColorOfMesh(startNodeNeighboursList[i], Color.magenta);
            }
        }
    }
    ...
    Node GetNodeBasedOnLocalPositionAndSetColorOfMesh(Vector3 nodePosition, Color meshColor, bool returnNode)
    {
        Node node = grid.getNodeBasedOnLocalPosition(nodePosition);
        grid.SetColorOfMesh(node, meshColor);

        if (returnNode) return node;
        else return null;
    }

    Node GetNodeBasedOnWorldPostionAndSetColorOfMesh(Vector3 nodeWorldPosition, Color meshColor, bool returnNode)
    {

        Node node = grid.GetNodeBasedOnWorldPosition(nodeWorldPosition);
        grid.SetColorOfMesh(node, meshColor);
        if (returnNode) return node;
        else return null;
    }
    ...
    bool CheckWhetherCurrentNodeIsGoal(Vector3 nodePostion, ref List<Node> finalPath)
    {
        ...
        {
            pathFound = true;
            Node checkedNode = GetNodeBasedOnLocalPositionAndSetColorOfMesh(goalNodePosition, Color.red, true);
    ...
    void GetAndShowFinalPath(Node node, ref List<Node> finalPath)
    {
        ...
        finalPath.Add(node);
        grid.SetColorOfMesh(node,Color.white);
    ...
    void SetNewGoalAndStartNodeAndItsPostions(Vector3 startNodeGridPosition, Vector3 goalNodeGridPosition)
    {
        ...
        startNode = GetNodeBasedOnWorldPostionAndSetColorOfMesh(startNodeGridPosition, Color.green, true);
        goalNode = GetNodeBasedOnWorldPostionAndSetColorOfMesh(goalNodeGridPosition, Color.red, true);
        goalNodePosition = goalNode.localPosition; ;
    }
}
